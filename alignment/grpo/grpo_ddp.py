...
def parse_args():
    ...
    parser.add_argument("--sft_checkpoint", type=str, required=False)
...
def setup_logging(checkpoint_dir,args):
    ...
    log_file = os.path.join(checkpoint_dir, f"train_lr{lr_str}_{timestamp}.log")
...
def main():
    ...
    logger.info(f"3. 日志/Checkpoint：checkpoint_dir={args.checkpoint_dir}")
    run_name = f"grpo" 
    ...
    train_loader=DataLoader(train_data,int(args.rollout_batch_size/args.group_size),shuffle=True,drop_last=True)
    tokenizer = AutoTokenizer.from_pretrained(args.tokenizer)
    model=AutoModelForCausalLM.from_pretrained(args.model,attn_implementation="flash_attention_2",torch_dtype=torch.bfloat16).to(train_device)
    ...
    Loadcheckpoint(args.sft_checkpoint,model,optimizer)
    logger.info(f'成功从{args.sft_checkpoint}加载SFT模型参数！')
    ...
    for step in range(1,args.n_grpo_steps+1):
        ...
        if args.loss_type=='no_baseline':
            _,raw_rewards,rewards_mean=compute_group_normalized_rewards(r1_zero_reward_fn,rollout_responses,repeated_ground_truths,args.group_size,args.advantage_eps,args.use_std_normalization)
            advantages=None
        else:
            advantages,raw_rewards,rewards_mean=compute_group_normalized_rewards(r1_zero_reward_fn,rollout_responses,repeated_ground_truths,args.group_size,args.advantage_eps,args.use_std_normalization)
        ...
        if step % args.log_interval == 0:
            token_entropy=[]
            response_mask=[]
            reward_acc=rewards_mean['total_reward_acc']
    ...
    batch_old_log_probs,cliprange
        ...
        if step % args.val_interval == 0:
            ...
            load_policy_into_vllm_instance(model,infermodel)
            val_sampling_params = SamplingParams(temperature=args.sampling_temperature,top_p=args.top_p,max_tokens=args.sampling_max_tokens,min_tokens=args.sampling_min_tokens)
            val_sampling_params.stop = ["</answer>"] 
            val_sampling_params.include_stop_str_in_output = True
            output=evaluate_vllm(infermodel,r1_zero_reward_fn,val_data['prompt'],val_sampling_params,val_data['correct_answer'],args.reward_path)

            reward_acc,answer_reward_acc,format_reward_acc,average_response_length,average_correct_response_length,average_incorrect_response_length=validate(output)
            ...
            if answer_reward_acc>max_acc:
                ...
                max_acc_steps=step
                model_save_path=Savecheckpoint(args.learning_rate,model,optimizer,step,args.checkpoint_dir)
