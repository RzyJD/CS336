...
def run_train_bpe1(
    ...
    **kwargs,
    ...
    # 保存词汇表到文件 (使用 pickle)
    with open("vocab_TinyStories_train.pkl", "wb") as f:
        pickle.dump(vocab, f)
    
    # 保存合并操作记录到文件 (使用 pickle)
    with open("merges_TinyStories_train.pkl", "wb") as f:
        pickle.dump(merges, f)
    return vocab, merges # 返回最终的词汇表和合并记录
if __name__=='__main__':
    run_train_bpe1('TinyStoriesV2-GPT4-train.txt',10000,['<|endoftext|>'])
